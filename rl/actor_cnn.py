...
class ActorCnn(nn.Module):
    ...
    def forward(self, state):
        ...
        x = self.features(img) # get features of img
        x = x.view(x.size(0), -1) # reshapes to (batch_size, n)
        x = torch.cat((x, pos.squeeze(), lie), dim=1) # create single input vector which we can pas to theta and club
...
class CriticCnn(nn.Module):
    ...
    def forward(self, state):
        img, pos, lie = state 
        x = self.features(img) # get features from image
        x = x.view(x.size(0), -1) # turn to (batch_size, n) shape
        x = torch.cat((x, pos.squeeze(), lie), dim=1) # create single tensor for passing to self.fc
